...
function updateBar(barData,svgName){
  ...
  var svg = d3.selectAll(svgName).selectAll(".wrapper")
 
  bars = svg.selectAll("rect")
    .data(barData, function(d){return d.letter})

  bars.enter().append("rect").transition().duration(500)
    .attr("class", "bar")
    .attr("x", function(d) { return x(d.letter); })
    .attr("width", x.rangeBand())
    .attr("y", function(d) { return y(d.score); })
  ...
  if(svgName==".topBar"){
    titleText = "Hours spent "+$("#activitySelect").val().toLowerCase()+" by profession"
  }else{
    titleText = "How your selection spends its time"
  }
