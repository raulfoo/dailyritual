...
namespace :fezzik do
  ...
  remote_task :pre_push_setup, :roles => :root_user do
    ...
    add_user("dailyritual") unless has_user?("dailyritual")
    run "ruby --version > /dev/null || apt-get install -y ruby"
    run "rsync --version > /dev/null || apt-get install -y rsync"
    run "mkdir -p /opt/#{app}/releases && chown -R dailyritual /opt/#{app}"
  end

  desc "performs post-push setup"
  remote_task :post_push_setup, :roles => :dailyritual_user do
  ...
  remote_task :generate_upstart_scripts, :roles => :dailyritual_user do
